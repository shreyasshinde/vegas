...
def get_source_point(paramIndex=0):
    if isConsoleTool():
        ...
        source_pts_fc = str(arcpy.GetParameterAsText(paramIndex));
    if source_pts_fc == "":
        raise Exception("Error: Input argument path to source point feature class cannot be empty.");
    else:
        return source_pts_fc;
...
def get_dest_point(paramIndex=0):
    if isConsoleTool():
        ...
        dest_pt_fc = str(arcpy.GetParameterAsText(paramIndex));
    if dest_pt_fc == "":
...
def simulate_for_worker(parameters):
    ...
    source_pts_fc_name = parameters["source_pts_fc_name"];
    dest_pts_fc_name = parameters["dest_pts_fc_name"];
    ...
    source_pts_fc_path = os.path.join(working_dir, fgdb_name, fds_name, source_pts_fc_name);
    dest_pts_fc_path = os.path.join(working_dir, fgdb_name, fds_name, dest_pts_fc_name);
    
    arcpy.AddMessage("Worker (" + worker_id + ") - fds path: " + fds_path);
    arcpy.AddMessage("Worker (" + worker_id + ") - fds path: " + fgdb_path);
    arcpy.AddMessage("Worker (" + worker_id + ") - source points fc path: " + source_pts_fc_path);
...
def simulate(source_pts_fc_path, dest_pts_fc_path, network_ds_path, randomness, iterations=1000, cores=4, working_dir=None, toolbox=None, output_routes_fc=None):
    '''
    ...
    # copy the source points and destination points fc to the feature dataset
    arcpy.AddMessage("Copying the source and destination points to feature dataset.");
    source_pts_fc = fds_path + "\\source";
    dest_pts_fc = fds_path + "\\destination";
    arcpy.CopyFeatures_management(source_pts_fc_path, source_pts_fc);
    arcpy.CopyFeatures_management(dest_pts_fc_path, dest_pts_fc);
    ...
    for i in xrange(cores):
        ...
        parameters["source_pts_fc_name"] = "source";
        parameters["dest_pts_fc_name"] = "destination";
...
def main():
    try:
        ...
        # simulate the paths
        output_routes_fc = simulate(source_pts_fc, dest_pts_fc, nd_path, rand_percent, iterations, cores, working_dir, toolbox, output_routes_path);
